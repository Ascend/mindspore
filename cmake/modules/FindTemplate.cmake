...
# ${_pkg}_FIND_LIBRARY_SUFFIXES - List of library suffixes (in lieu of CMAKE_FIND_LIBRAY_SUFFIXES)
# Functions
...
if(NOT ${_pkg}_NO_PKGCONFIG)
  ...
  if(PkgConfig_FOUND)
    ...
    _debug_print("${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}"
                 "Skipping PkgConfig since no pkg-config excutable could not be found.")
  endif()
endif()
...
set(_${_pkg}_PATH_SUFFIXES ${${_pkg}_INCLUDE_PATH_SUFFIXES})
...
if(NOT ${_pkg}_INCLUDE_DIR)
  ...
  _debug_print_var("${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "_${_pkg}_PATH_SUFFIXES")
  ...
  find_path(
    ...
    PATH_SUFFIXES ${_${_pkg}_PATH_SUFFIXES})
...
foreach(config DEBUG RELEASE)
  ...
  if(${_pkg}_LIBRARY_DIR_${config})
    ...
    elseif(${_pkg}_LIB_SYSTEM_PATHS)
      list(APPEND _${_pkg}_LIBRARY_SEARCH_DIRS_${config} PATHS ${${_pkg}_LIB_SYSTEM_PATHS})
...
if(${_pkg}_EXECUTABLE_DIR)
  ...
  if(${_pkg}_ROOT)
    ...
    _update_library_search_dirs_with_prebuilt_paths(_${_pkg}_EXECUTABLE_SEARCH_DIRS "${${_pkg}_ROOT}")
  elseif(_ENV_${_pkg}_ROOT)
    list(APPEND _${_pkg}_EXECUTABLE_SEARCH_DIRS ${_ENV_${_pkg}_ROOT})
    _update_library_search_dirs_with_prebuilt_paths(_${_pkg}_EXECUTABLE_SEARCH_DIRS "${_ENV_${_pkg}_ROOT}")
  endif()
      ...
      if("${${_pkg}_${_comp}_TYPE}" STREQUAL "LIBRARY")
        if(NOT TARGET ${_${_pkg}_tgt})
          if(${_pkg}_${_comp}_HEADER_ONLY)
            ...
            add_library(${_${_pkg}_tgt} SHARED IMPORTED)
          endif()
        endif()
        ...
        if(NOT TARGET ${_${_pkg}_tgt})
          add_executable(${_${_pkg}_tgt} ${_${_pkg}_libtype} IMPORTED)
        endif()
      endif()
